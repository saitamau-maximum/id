...
// 外部プロバイダ (GitHub Org Team / Discord Guild Role) のキャッシュ
...
export const externalRolesRelations = relations(externalRoles, ({ many }) => ({
	grantConditions: many(externalRoleGrantConditions),
}));

// 外部ロール付与条件: 同じ external_role_id の行が複数あれば OR セマンティクス
export const externalRoleGrantConditions = sqliteTable(
	"external_role_grant_conditions",
	{
	...
	(table) => [
		primaryKey({ columns: [table.externalRoleId, table.requiredRoleId] }),
		index("external_role_grant_conditions_required_role_idx").on(
			table.requiredRoleId,
		),
	],
);

export const externalRoleGrantConditionsRelations = relations(
	externalRoleGrantConditions,
	({ one }) => ({
		externalRole: one(externalRoles, {
			fields: [externalRoleGrantConditions.externalRoleId],
