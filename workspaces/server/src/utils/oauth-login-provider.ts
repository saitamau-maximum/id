...
export abstract class OAuthLoginProvider {
	...
	public async loginHandlers(
		c: Context<HonoEnv>,
		query: v.InferOutput<typeof OAuthLoginProvider.LOGIN_REQUEST_QUERY_SCHEMA>,
	) {
		this.env = c.env;
		this.honoVariables = c.var;
		this.origin = new URL(c.req.url).origin;

		const { continue_to, invitation_id, from: login_origin } = query;
	...
	public async callbackHandlers(
		c: Context<HonoEnv>,
		query: v.InferOutput<
			typeof OAuthLoginProvider.CALLBACK_REQUEST_QUERY_SCHEMA
		>,
	) {
		this.env = c.env;
		this.honoVariables = c.var;
		...
		// login origin check
		const { success: isValidLoginOrigin, output: loginOrigin } = v.safeParse(
		...
		if (cookieJwt) {
			...
			if (payload) {
				loggedInUserId = (payload as HonoEnv["Variables"]["jwtPayload"]).userId;
			}
		}
