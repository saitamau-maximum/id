	...
	implements IExternalRoleConditionRepository
{
	...
	async listAll(): Promise<ExternalRoleCondition[]> {
		...
		return rows.map((row) => {
			if (!v.is(OAuthProviderId, row.externalRole.providerId)) {
				throw new Error(`Invalid provider ID: ${row.externalRole.providerId}`);
			}
			return {
				id: row.id,
				providerId: row.externalRole.providerId,
